...
class AtomicCommitProtocol:
    ...
    def get_reward(cls, global_state: list[StateMachine]) -> int:
        ...
        for sm in global_state:
            if sm.crashed:
                if sm.get_final_state() is State.Lost:
                    continue
            reach_final_nodes.append(sm)

        # Rule 1: all uncrashed nodes must reach a final decision
        if any(not sm.get_final_state().is_final for sm in reach_final_nodes):
        ...
        final_states = [sm.get_final_state() for sm in reach_final_nodes]
        ...
        # Acceptable cases
        # TODO: maybe we should encourage Commit if possible, at least we can test the liveness
        if reach_final_nodes[0].get_final_state() is State.Commit:
